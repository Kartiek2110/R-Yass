...
const EMAILJS_PUBLIC_KEY = 'VrdlMzPF-fcYAEuqL';
const EMAILJS_SERVICE_ID = 'service_0fwgtyg'; 
const EMAILJS_TEMPLATE_ID = 'template_vjlaro1';

// Google Apps Script URL (SIMPLE and WORKING approach!)
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxZtkA4RJdVOtvJ1j6Mu2SD-qL_sa4dTMfmGkbxQKULz4qwrU9jfCxd_qsOcQVvQK3gWA/exec';
...
export const sendEmail = async (formData: ContactFormData) => {
  try {
    ...
    if (errorText?.includes('service ID not found')) {
      throw new Error('EmailJS Service ID is incorrect. Please check your EmailJS dashboard.');
    } else if (errorText?.includes('template ID not found')) {
      throw new Error('EmailJS Template ID is incorrect. Please check your EmailJS dashboard.');
    } else if (errorText?.includes('public key')) {
      throw new Error('EmailJS Public Key is incorrect. Please check your EmailJS account settings.');
    }
    
    throw new Error(`Email sending failed: ${errorMessage || errorText || 'Unknown error'}`);
  }
};
...
export const saveToGoogleSheets = async (formData: ContactFormData) => {
  try {
    ...
    return { success: true, message: 'Data sent to Google Sheets via Apps Script' };
...
export const submitContactForm = async (formData: ContactFormData) => {
  ...
  const results = {
    email: null as { success: boolean; response?: any } | null,
  ...
  try {
    ...
    results.errors.email = error instanceof Error ? error : new Error(String(error));
  }
  ...
  try {
    ...
    results.errors.sheets = error instanceof Error ? error : new Error(String(error));
  }
  ...
  if (emailSuccess && sheetsSuccess) {
    ...
    results.message = '✅ Email sent successfully! (Google Sheets had an issue, but your inquiry was received)';
    return results;
  } else if (sheetsSuccess) {
    results.message = '✅ Data saved successfully! (Email had an issue, but we got your information)';
    return results;
  } else {
    ...
    throw new Error('Unable to process your inquiry. Please try again or contact us directly.');
  }
}; 